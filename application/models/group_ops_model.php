...
class Group_ops_model extends CI_Model
{
	 ...
	 */
	public function alllist($offset,$num,$gop_state=0)
	{
		
		 $sql="select group_name,realname,gop_state,gop_id from $this->table,users,git_groups where $this->table.change_id=users.id and $this->table.group_id=git_groups.group_id and $this->table.gop_state=$gop_state order by gop_id desc limit $offset,$num";
		return $this->db->query($sql)->result_array();
	}
	public function alllist_count($gop_state=0)
	{
		return $this->db->query("select gop_id from $this->table where gop_state=$gop_state")->num_rows();
	}
	/**
	 * 获取一个申请操作的详细信息
	 */
	public function get_op_info($gop_id)
	{
		...
		$usersql="select username,realname from git_groups_user as g,users where users.id=g.user_id and group_id=$group_rs[group_id]";
		$all_user=$this->db->query($usersql)->result_array();
		$data['grop_rs']=$gop_rs;
