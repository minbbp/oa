 ...
 * @author minbbp
 */
class Codeonline_ungent extends MY_Controller
{
	...
	{
		$this->load->view('codeonline_apply/ungent_reject',array('a_id'=>$a_id));
	}
	/**
	 * 驳回用户申请信息
	 * @param number $a_id
	...
	public  function savereject($a_id)
	{
		...
		if($this->cam->save($data,$a_id))
		{
			...
			else
			{
				...
				log_message('error','紧急上线测试审批失败a_id='.$a_id);
			}
		}
		else
		{
